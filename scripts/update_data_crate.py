...
def main(version):
    ...
    for datafile in crate.get_by_type("Dataset"):
        ...
        if datafile.id != "./" and not datafile.id.startswith("http"):
            ...
            date_modified = datetime.datetime.fromtimestamp(stats.st_mtime).strftime(
                "%Y-%m-%d"
            )
            if file_path.is_dir():
                ...
                data_props.update(
                    {"contentSize": stats.st_size, "dateModified": date_modified}
                )
            else:
                rows = 0
            ...
            if datafile.get("encodingFormat") == "text/csv":
                ...
                schema_props[
                    "name"
                ] = f"Frictionless Table Schema for {data_props['name']} dataset"
    ...
    for action in actions:
        action_dates = []
        props = action.properties()
        for result in listify(props["result"]):
    ...
    crate.write_detached("ro-crate-metadata.json")
